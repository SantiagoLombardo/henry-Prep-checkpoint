...
function menorMayor(numeros) { 
  ...
  var minMax=[numeros[0],numeros[0]];
  numeros.map(function(array){
    if(array<minMax[0]){
      return minMax[0]=array;
    }
    if(array>minMax[1]){
      return minMax[1]=array;
    }
  })
  return minMax;
}
...
function stringMasLarga(strings) {
  ...
  strings.forEach(function(strings) {
    if (longString.length<strings.length){
      longString=strings;
    }
  });
  return longString;
...
function sumArray(array, n) {
  ...
  var mach = 0;
  for(var i=0;i<array.length && mach===0 ;i++){
    for(var j=0; j!==i && j<array.length && mach===0 ;j++){
...
function pluck(array, propiedad) { 
  ...
  return newArray = array.map(function(element){
...
function crearClasePersona() {
  ...
  class Persona {
    ...
    getFriends() {
      ...
      return this.amigos.map(function(amigos){
    ...
    getPromedioEdad() {
      ...
      var promedio = this.amigos.reduce(function(acc, amigos){
...
function filtrar(funcion) {
  ...
  //   return p.price >= 50;
  // }) => [{price: 100, name:'tv'}]
  Array.prototype.filtrar = function filtrar (fn){
    var resultado = [];
